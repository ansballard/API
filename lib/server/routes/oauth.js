...
const clients = [
  "https://modwat.ch", // regular site
  "http://local.modwat.ch", // local uploader, name feels wrong
  "http://uploader.modwat.ch", // local uploader, makes more sense
  "https://modwatch-staging.firebaseapp.com" // staging
];
...
module.exports = function oauth(app, { jwtSecret, env }) {
  ...
  app.get("/oauth/verify", verify(jwtSecret), (req, res) => {
    res.sendStatus(200);
  });

  app.post("/oauth/authorize", (req, res) => {
    const query = serialize(req.query);
    const scopes = req.body.scopes ? req.body.scopes.split(",") : [];
    ...
    Modlist.findOne({ username: `${req.body.username}` })
      .then(
        modlist =>
          modlist && modlist.validPassword(`${req.body.password}`)
            ? modlist
            : Promise.reject(401)
      )
      .then(
        modlist =>
          scopes.filter(s => modlist.roles.includes(s)).length ===
          scopes.filter(s => s !== "upload").length
            ? modlist
            : Promise.reject(401)
      )
      .then(modlist => {
        ...
        const exp =
          iat + 3600 * 1000 * (scopes.includes("upload") ? 24 * 30 : 1);
        const token = {
  ...
  app.delete("/oauth/user/:username/delete", verify(jwtSecret), (req, res) => {
    if (
...
function verifyRole(role, jwtSecret) {
  return (req, res, next) => {
    verify(jwtSecret)(req, res, () => {
      if (req.access_token.scopes.includes(role)) {
        next();
      } else {
        res.sendStatus(403);
      }
