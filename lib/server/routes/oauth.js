...
module.exports = function oauth(app) {
  ...
  app.post("/oauth/authorize", (req, res) => {
    ...
    .then(modlist => {
      ...
      res.redirect(`${decodeURIComponent(req.query.redirect_uri)}oauth/access_token/${encodeURIComponent(encode(token, process.env.JWTSECRET))}/token_type/Bearer/expires_in/3600`);
    })
    .catch(e => {
      console.log(e);
...
function verify(req, res, next) {
  try {
    const token = decode(req.get("Authorization").slice(7), process.env.JWTSECRET);
